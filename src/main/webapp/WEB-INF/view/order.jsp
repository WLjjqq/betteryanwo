...
<div class="am-container header">
	...
	<ul class="message-r">
		...
		<div class="topMessage mini-cart">
			<div class="menu-hd"><a id="mc-menu-hd" href="#" target="_top"><i class="am-icon-shopping-cart  am-icon-fw"></i><span>购物车</span><strong id="J_MiniCartNum" class="h">{{datas[0]}}</strong></a></div>
		</div>
	</ul>
</div>
					...
					<li class="user-addresslist"  v-cloak>
						<div class="address-left" >
							<div class="user DefaultAddr">
											...
											<span class="buy-user">{{address.consignee}}</span><br/>
												<span>用户手机号：&nbsp &nbsp &nbsp </span>
											<span class="buy-phone">{{address.consigneeTel}}</span>
											</span>
							</div>
							<div class="default-address DefaultAddr">
								...
								<span class="buy--address-detail">
									...
									<span class="dist" >{{address.county}}</span>区</br>
											<span class="street" >{{address.site}}</span>
										</span>
						...
						<div class="new-addr-btn">
							...
							<a v-bind:id="address.addressId" href="javascript:void(o);" onclick="update_address(this);" v-bind:value="address.addressId">编辑</a>
							<span class="new-addr-bar">|</span>
							<a v-bind:id="address.addressId" href="javascript:void(0);" v-bind:value="address.addressId" onclick="delClick(this);">删除</a>
						</div>
					</li>
				</ul>
			</div>
...
<script type="text/javascript">
       ...
       methods: {
           showData: function () {
               $.ajax({
                   ...
                   success: function (result) {
                       for (var i = 0; i < result.data.length; i++) {
                           address.addresss.push(result.data[i]);
                       }
   ...
   function build_address_table(result) {
       var addressItems = result.data;
       //alert(addressItems[0]);

	   var cart=$(".cart_sum").append(addressItems[0].cid);//购物车的数量
		var province=$(".province1").append(addressItems[0].province);//省份province
	   var city=$(".city1").append(addressItems[0].city);//市city
       var dist=$(".dist1").append(addressItems[0].county);//区county dist
       var street=$(".street1").append(addressItems[0].site);//地址site street
	   var user=$(".buy-user1").append(addressItems[0].consignee);//收货人 buy-user  consignee
	   var pphone=$(".buy-phone1").append(addressItems[0].consigneeTel);//收货人电话 buy-phone  consigneeTel
   }
...
//点击保存按钮，保存地址
	$("#address_save").click(function () {
		$.ajax({
			url:"http://localhost:9000/address/saveAddress/2",
            type:"POST",
