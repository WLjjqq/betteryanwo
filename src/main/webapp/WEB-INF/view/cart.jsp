...
<script type="text/javascript">
	...
	$.each(cartItems,function(index,item){
		...
		var checkBoxTd = $("<td><input type='checkbox' class='check_item'/></td>");
			...
			var number=$("<td></td>").addClass("quantity-btn")
						.append($("<button></button>").addClass("btn btn-default btn-sm minus").attr("id","minus").attr("cartItemId",item.id).append("-"))
                        .append(" ")
						.append($("<input style='width:45px;height:20px'/>").addClass("form-control-static quantity-display").val(item.itemNum))
                        .append(" ")
	...
	function build_cart_count(result){
		$("#sum_goods").empty();
		$("#sum_price").empty();
		$("#sum_goods").append("已选择"+" "+result.data[0].snum +"  件商品");
	...
	$("#cart_delete_all_btn").click(function(){
		...
		//修改相应的价格。
		var price=$(this).parents("tr").find("td:eq(3)").text().substring(1);
		//alert(price);
		var a=price*newCartNum;
		$(this).parents("tr").find("td:eq(5)").empty().append("￥").append(a);
		setTotal();
		});
